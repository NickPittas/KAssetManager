    ...
    , assetsModel(nullptr)
{
    ...
    // Load LivePreview cache size setting (default to 1024 if unset)
    {
        QSettings s("AugmentCode", "KAssetManager");
        int cacheSize = -1;
        if (s.contains("LivePreview/MaxCacheEntries")) {
            cacheSize = s.value("LivePreview/MaxCacheEntries").toInt();
        } else {
            cacheSize = 1024;
...
void MainWindow::releaseAnyPreviewLocksForPaths(const QStringList& paths)
{
    ...
    // Embedded File Manager preview: delegate to FileManagerWidget
    if (fileManagerWidget) {
        fileManagerWidget->releaseAnyPreviewLocksForPaths(paths);
    }
...
void MainWindow::showPreview(int index)
{
    ...
    if (!previewOverlay) {
        ...
        /* no external stop; handled internally by showAsset/showSequence */
    }
...
bool MainWindow::eventFilter(QObject *watched, QEvent *event)
{
    ...
    if (((fmGridView && (watched == fmGridView || watched == fmGridView->viewport())) ||
        ...
        if (keyEvent->key() == Qt::Key_Space && !keyEvent->isAutoRepeat()) {
            ...
            if (fileManagerWidget) fileManagerWidget->onFmOpenOverlay();
            return true;
        }
    }
