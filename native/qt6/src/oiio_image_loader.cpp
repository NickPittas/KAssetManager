...
QImage OIIOImageLoader::loadImage(const QString& filePath, int maxWidth, int maxHeight) {
    ...
    if (isHDR) {
        qDebug() << "[OIIOImageLoader] HDR image detected, applying tone mapping";
        ...
        // Apply tone mapping
        return toneMapHDR(pixels.data(), width, height, targetChannels);
    } else {
...
QImage OIIOImageLoader::toneMapHDR(const float* data, int width, int height, int channels, float exposure) {
#ifdef HAVE_OPENIMAGEIO
    qDebug() << "[OIIOImageLoader] Tone mapping HDR image:" << width << "x" << height;
